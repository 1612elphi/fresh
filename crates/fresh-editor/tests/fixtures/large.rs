    ...
    pub fn handle_plugin_command(&mut self, command: PluginCommand) -> AnyhowResult<()> {
        match command {
            ...
            PluginCommand::CreateVirtualBufferWithContent {
                ...
                // Now set the content
                match self.set_virtual_buffer_content(buffer_id, entries, Vec::new(), Vec::new()) {
                    Ok(()) => {
            ...
            PluginCommand::CreateVirtualBufferInSplit {
                ...
                if let Some(pid) = &panel_id {
                    if let Some(&existing_buffer_id) = self.panel_ids.get(pid) {
                        ...
                        if self.buffers.contains_key(&existing_buffer_id) {
                            ...
                            if let Err(e) =
                                self.set_virtual_buffer_content(existing_buffer_id, entries, Vec::new(), Vec::new())
                            {
                ...
                // Set the content
                if let Err(e) = self.set_virtual_buffer_content(buffer_id, entries, Vec::new(), Vec::new()) {
                    tracing::error!("Failed to set virtual buffer content: {}", e);
                    return Ok(());
                }
            ...
            PluginCommand::SetVirtualBufferContent { buffer_id, entries, scroll_regions, border_regions } => {
                match self.set_virtual_buffer_content(buffer_id, entries, scroll_regions, border_regions) {
                    Ok(()) => {
                        tracing::debug!("Set virtual buffer content for {:?}", buffer_id);
                    }
                    Err(e) => {
